...
class BERTClassificationHeadWithAttribute(nn.Module):
    """Head for sentence-level classification tasks."""
    # type b: for text representation
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size + config.attr_dim * 3, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size, config.num_labels)

    def forward(self, hidden_states, attrs):
        #  *attrs : unpacking, printing data in a raw format
