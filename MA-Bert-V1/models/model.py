...
class MAAModel(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.usr_beh_embed = nn.Embedding(self.cus_config.num_labels, self.cus_config.attr_dim)
        self.usr_beh_embed.weight.requires_grad = True
        init.uniform_(self.usr_embed.weight, a=-0.25, b=0.25)
    ...
    def forward(
        ...
        usrs, prds, usrs_beh = attrs # (bs, ) * 2
        usr = self.usr_embed(usrs) # (bs, attr_dim)
        prd = self.prd_embed(prds) # (bs, attr_dim)
        usr_beh = self.usr_beh_embed(usrs_beh)
        if self.type == 'b':
            hidden_state = self.dropout(last_output)
            outputs = self.classifier(hidden_state, [usr, prd, usr_beh])
        elif self.type == 'a':
            ...
            hidden_state = self.fusion(last_output, [usr, prd, usr_beh])
            ...
            for i, mmalayer in enumerate(self.ATrans_decoder):
                hidden_state = mmalayer([usr, prd, usr_beh, t_self], hidden_state, extend_attention_mask)
            hidden_state = self.dropout(hidden_state)
            outputs = self.classifier(hidden_state)
