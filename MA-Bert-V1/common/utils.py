...
def calculate_user_behavior(data):
    user_rating_behavior = defaultdict()
    for d in data:
        for k, v in d.items():
            user_rating_behavior[k] = round(sum(v) / len(v))
    return user_rating_behavior

def save_user_behavior_to_local(config, object='usr', feature='behavior'):
    processor = DATASET_MAP[config.dataset]()
    user_rating_history = processor.get_user_rating_history()
    user_behavior = calculate_user_behavior(user_rating_history)
    torch.save(user_behavior, os.path.join(DATASET_PATH_MAP[config.dataset], '{}_{}.pt'.format(object, feature)))
    return user_behavior

def load_user_behavior_from_local(path, object='usr', feature='behavior'):
    return torch.load(os.path.join(path, '{}_{}.pt'.format(object, feature)))

def load_user_behavior(config):
    try:
        usr_behavior = load_user_behavior_from_local(DATASET_PATH_MAP[config.dataset], object='usr', feature='behavior')
    except:
        save_user_behavior_to_local(config, object='usr', feature='behavior')
        usr_behavior = load_user_behavior_from_local(DATASET_PATH_MAP[config.dataset], object='usr', feature='behavior')
...
def load_document4baseline_from_local(config):
    try:
        ...
        config.num_usrs = len(usr_stoi)
        config.num_prds = len(prd_stoi)
        config.TRAIN.num_train_optimization_steps = int(len(train_dataset) / config.TRAIN.batch_size / config.TRAIN.gradient_accumulation_steps) * config.TRAIN.max_epoch
